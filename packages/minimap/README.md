...
使用 `@overworld/scene` 的 `playerPositionRef` / `playerRotationRef`,也可以传任何
形状相同的对象。
...
```ts
import { projectToCanvas, projectionScale } from '@overworld/minimap'
...
依赖 `@overworld/core`(仅类型);peer:`react`、`zustand`。不依赖 three.js。
