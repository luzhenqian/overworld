...
## 本版本新增:场景级加载状态 + 分区流式加载

`useLoadingStore` 之外,新增一套面向"进场景到看到第一帧"全过程的阶段状态,
以及按玩家位置排序的分区(zone)预加载。

```ts
import { useSceneLoadStore, useZoneStreaming, FirstFramePhase } from '@overworld-engine/loading'

useSceneLoadStore.getState().completePhase('module')   // 阶段推进(module→geometry→texture→first-frame→ready)
useZoneStreaming(zones, playerPosRef)                   // 按到玩家距离由近及远预加载各分区清单
```

- `useSceneLoadStore` — 阶段状态机 `SCENE_PHASES`
  (`idle`→`module`→`geometry`→`texture`→`first-frame`→`ready`),
  `aggregateSceneProgress(phases)` 为四个真实加载阶段的纯函数均值。
- `useZoneStreaming(zones, playerPosRef)` — Hook,按
  `orderZonesByDistance` 由近及远预热每个 `ZoneManifest`,失败写入
  `useSceneLoadStore` 的 `errors`。
- `<FirstFramePhase />` — 挂在 Canvas 内,geometry 阶段完成后的首帧自动
  标记 `first-frame` 完成。
- `installSceneLoadDebugHandle()` — 仅开发环境,把加载状态镜像到
  `window.__overworldSceneLoad`,方便 Playwright 等 E2E 直接轮询 `phase`。
