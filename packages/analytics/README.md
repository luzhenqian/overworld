...
```ts
import { configureAnalytics, ga4Provider, clarityProvider, consoleProvider, track, trackPage } from '@overworld-engine/analytics'
...
import { bindAnalyticsToBus } from '@overworld-engine/analytics'
...
依赖 `@overworld-engine/core`(仅类型:`EventBus`);peerDependency 为 `react`。
