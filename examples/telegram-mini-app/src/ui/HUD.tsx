...
/**
 * HUD 面板底色与强调色走 CSS 变量,由 Telegram themeParams 在启动时写入
 * (见 main.tsx 的 applyTelegramTheme),浏览器直开时用回退值。
 */
const panelStyle: React.CSSProperties = {
  background: 'var(--hud-panel-bg, rgba(10, 14, 26, 0.82))',
  border: '1px solid var(--hud-panel-border, #2b3652)',
  borderRadius: 10,
  padding: '10px 14px',
  color: 'var(--hud-text, #dbe4ff)',
  fontSize: 13,
  fontFamily: 'system-ui, sans-serif',
  pointerEvents: 'auto',
}

const accent = 'var(--hud-accent, #facc15)'
...
function QuestTracker() {
  ...
  return (
    <div style={{ ...panelStyle, width: 240 }}>
      <div style={{ fontWeight: 700, marginBottom: 6, color: accent }}>任务</div>
      {entries.map((quest) => {
...
export function HUD() {
  return (
    <div style={{ position: 'absolute', inset: 0, pointerEvents: 'none' }}>
      ...
      {/* 触屏环境(shouldShowTouchControls)才挂载虚拟摇杆 */}
      {showTouchControls && (
