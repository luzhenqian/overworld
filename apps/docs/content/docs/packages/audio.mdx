...
- 缺省 `htmlAudioBackend`(已导出):`new Audio(url)`,与历史行为一致;
- 微信小游戏:`backend: createWeappAudioBackend()`
  (来自 `@overworld-engine/adapters-weapp`,映射 `wx.createInnerAudioContext`)。
- `silentBackend` 是无 DOM、SSR 与测试环境的静默实现，满足相同契约但不播放声音。
