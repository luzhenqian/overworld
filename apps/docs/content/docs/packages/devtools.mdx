---
title: '@overworld-engine/devtools'
description: 开发期工具:内容静态校验、JSON Schema、事件总线日志与剖析
---
...
已对照 `@overworld-engine/dialogue` 引擎实现确认:**没有 `responses` 也没有 `next`
...
## 场景与多场景项目校验(validateScene / validateSceneProject)

`validateScene(json, options?)` 静态校验单份 `SceneJson`(编辑器 `exportScene()`
的产物 / `<SceneFromJson>` 的输入):id 唯一、必填字段齐全、`scale`/`collisionRadius`
非负、装饰组结构合法等为 error,空场景与未知 `modelPath`(仅当传入
`knownModelPaths`)为 warning。

`validateSceneProject(json, options?)` 校验编辑器 `exportProject()` 产出的
**多场景项目** `SceneProjectJson`(`{ version?, scenes: [{ id, name, scene }], activeSceneId? }`):

- **项目结构(error)**:根不是对象;`scenes` 不是数组;空项目(`scenes: []`);
  条目不是对象或缺 `id` / `name` / `scene`;**scene id 重复**;**scene 名字重复**;
  `activeSceneId` 不是字符串或指向不存在的 scene id。
- **逐场景**:对每个 `scenes[i].scene` 跑上面的 `validateScene`,把结果**归并到
  该关卡** —— issue 的 `source` 置为该 scene 的 id,原来的实体级来源(如
  `npc:guide`)折进 `path`,不丢定位信息。

```ts
import { validateSceneProject, formatReport } from '@overworld-engine/devtools'

const report = validateSceneProject(useEditorStore.getState().exportProject(), {
  knownModelPaths: ['/models/guide.glb'],   // 可选:转发给每个内嵌场景的 validateScene
})
if (!report.ok) console.error(formatReport(report))
```

返回与其它校验器同样的 `ValidationReport`(`{ issues, errors, warnings, ok }`),
纯函数、从不抛出。选项 `SceneProjectValidationOptions` 继承 `SceneValidationOptions`
(即 `knownModelPaths`),会施加到项目里的每一个场景。
...
- 场景:`sceneConfigSchema`(单份 `SceneJson`)与 `sceneProjectSchema`
  (多场景 `SceneProjectJson`,内嵌 `sceneConfig` 形状);两者自包含、可配合
  `validateScene` / `validateSceneProject` 使用
- `contentBundleSchema`:`{ dialogues?, quests?, items?, achievements? }`
  整包(与 `validateContent` 的输入同形)
- `allContentSchemas`:按名字迭代所有 schema;`schemaFor('quests')` /
  `schemaFor('scene')` / `schemaFor('scene-project')` 直接取某一类的 schema
...
统计每个事件的发射次数与**同步分发耗时**——即所有监听器同步执行的时间;
监听器里发起的异步工作不计入。每个事件累计
...
profiler.top(3)                  // 最贵的 3 个事件(默认按 totalMs)
profiler.top(3, 'count')         // 或按发射次数
...
| 成员 | 说明 |
| --- | --- |
| `stats()` | 每事件统计快照(`Record<string, EventStats>`,返回副本) |
| `top(n?, by?)` | 前 `n`(默认 5)个事件,按 `'totalMs'`(默认)或 `'count'` 倒序 |
| `reset()` | 清空已收集的统计,剖析继续 |
| `stop()` | 还原原始 `emit` 并停止收集;幂等 |
| `report()` | 人类可读的对齐表格,按 `totalMs` 倒序 |

行为细节:

- `options.now` 可注入时钟(默认 `performance.now`),测试里注入假时钟即可
  得到确定性的耗时断言。
- **重复剖析同一总线会链式包裹**:第二个 profiler 包住第一个的 emit(并打
  warning),两者都正常统计(外层的耗时含内层的少量记账开销)。此时
  `stop()` 必须按 **LIFO** 顺序调用(后启动的先 stop)——乱序 stop 会把一个
  过期的 emit 还原回去,导致叠在上面的 profiler 悄悄失效。
