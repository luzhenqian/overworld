...
- **`interact()` / `useInteractKey(key = 'e', { isInputBlocked })`** —
  把"按 E 交互"翻译成总线上的 `entity:interact` 事件(当前 3.2.x 同载荷双发
  已弃用的 `interact`;新代码只订阅前者)。
...
如果该 id 同时由 `BaseNPC` 跟随 ref，不要再用 `AgentNPC` 渲染第二份可视对象。
...
详见[密集世界指南](/docs/guides/dense-world)。
