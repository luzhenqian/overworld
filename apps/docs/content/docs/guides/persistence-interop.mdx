---
title: 持久化互操作
description: 引擎本地持久化与服务端权威存档的组合模式:非权威切片、key 前缀隔离、RestStorage 对接
---
...
上面讲的是"权威数据 vs 本地缓存"的组合模式,前提是持久化后端本身可靠
(localStorage / 你的服务器)。如果你要的是**单机游戏的本地文件存档**在
强杀进程/断电时不损坏——那是另一个问题,`persistOptions`/`createSaveSlots`
不解决它。见 [`core` 包](/docs/packages/core)的存档文件原语
(`commitSlot`/`recoverSlot`,配合 `@overworld-engine/adapters-savefile` 的
桌面文件后端)。
