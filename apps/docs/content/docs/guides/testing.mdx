...
- **随机数**:任何依赖 `Math.random()` 的构造函数(掉落表、世界生成、
  战斗随机数)都应该把 `RngSource` 当可选依赖接受;测试传
  `createSeededRng(seed)`(`@overworld-engine/core`),同种子同结果。
...
内核单测(读 store)和 E2E(读 DOM/testid)都测不到**装配层**的 bug —— 一个
...
`renderHook` 挂载单个 hook 跑它的 `useEffect`(基于 `react-test-renderer`,
