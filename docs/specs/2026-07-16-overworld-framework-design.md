...
任务 + 物品 + 成就)时,直接依赖 `@overworld/*` 包,只需编写**游戏内容数据**和**玩法专属系统**。
...
1. **单一大包 `@overworld/engine`** — 上手简单,但边界模糊、按需引入困难,违背"很多通用模块"的要求。
2. **多包 monorepo(选定)** — 每个系统一个包,职责单一、可独立测试、可按需组合;跨包只依赖
   `@overworld/core` 的契约(事件总线/注册表),扩展性最好。
...
overworld/
├── packages/
│   ├── core            @overworld/core            类型化事件总线、条件/效果注册表、持久化辅助、公共类型
│   ├── input           @overworld/input           键盘优先级层级栈(modal > dialogue > panel > game)
│   ├── scene           @overworld/scene           3D 世界层:SceneShell、BaseNPC/BaseBuilding、Player、
│   │                                              跟随相机、圆形碰撞、邻近检测、GLTF 加载、主题、传送门
│   ├── dialogue        @overworld/dialogue        无头对话引擎(对话树 + 条件/效果解释器)
│   ├── quest           @overworld/quest           无头任务状态机(目标/触发/奖励,事件驱动)
│   ├── inventory       @overworld/inventory       无头物品/背包引擎
│   ├── achievements    @overworld/achievements    无头成就引擎(订阅事件总线自动解锁)
│   ├── tutorial        @overworld/tutorial        无头教程步骤引擎
│   ├── audio           @overworld/audio           BGM/音效管理(场景→曲目映射、自动播放策略处理)
│   ├── notifications   @overworld/notifications   Toast / Alert 无头通知队列
│   ├── loading         @overworld/loading         资源加载进度、场景预加载
│   └── analytics       @overworld/analytics       埋点抽象(provider 可插拔,内置 GA4/Clarity)
├── examples/
...
**系统包之间互不依赖**。所有跨系统通信走 `@overworld/core` 的事件总线;所有数据驱动的
...
游戏通过 **declaration merging** 扩展事件表(`declare module '@overworld/core' { interface
...
- `Player.tsx`:参数化——`modelUrl?`(无模型时回退胶囊体)、`animationMap?({idle,walk,run})`、
  ...
  **移除 questStore.onMoved 调用**。键盘输入读 @overworld/input?否——保持零内部依赖,
...
5. 场景里放一个可拾取物品,拾取进背包,完成"收集"目标,发奖励(效果注册表加金币),
   弹 Toast,解锁一个成就。
全程只使用 `@overworld/*` 公开 API,不允许 deep import。
...
- 版本:独立版本号,初始 0.1.0;命名统一 `@overworld/<name>`。
