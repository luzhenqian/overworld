...
全部 23 个包按上图分层。规则只有一条但必须严格遵守:**系统包之间禁止 import,只能通过
core 的事件总线和注册表间接协作**。这是从 degener-city 中吸取的教训——那里的 questStore
直接 import 了 8 个玩法 store,导致任务系统无法复用。
...
- 确定性(v1.2 起):全家桶可注入 `clock`/`scheduler`/`random`/`events`,同 seed 重放
  全等 —— 既是防作弊基础,也让带时间/随机的逻辑在单测中确定可断言。
- 3D / 联机 / 编辑器 / 多端:渲染与集成部分靠各示例的 Playwright E2E 作为验收
  (starter / dungeon / scene-authoring / content-packs,及四个端模板);store 驱动
  断言是官方推荐路径,详见测试指南。
