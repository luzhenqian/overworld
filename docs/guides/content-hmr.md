...
@overworld/devtools 的 `validateContent` 是纯函数,天然适合放在 accept
...
if (import.meta.hot) {
  import.meta.hot.accept('./content', async (mod) => {
    if (!mod) return
    const devtools = await import('@overworld/devtools')
    const bundle = {
...
- **persist 开启时留意版本**:引擎持久化的是进度而非定义,热更定义不会
  ...
  `persistOptions` 的 `version` + `migrate`(@overworld/core)迁移旧存档,
