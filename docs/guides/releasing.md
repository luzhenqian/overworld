...
所有 `@overworld/*` 包配置为 **fixed 版本组**,始终以同一版本号一起发布(锁步版本)。
...
- 消费 `.changeset/*.md`,按最高 bump 级别更新所有 `@overworld/*` 包的版本号
...
`.changeset/config.json` 中 `"fixed": [["@overworld/*"]]` 使全部框架包版本永远一致,
用户可以放心地把所有 `@overworld/*` 依赖固定在同一版本。私有应用与示例
